...
def delete_accel_less_than_30_velo(accel_type_str, accel_type_array, mean_velo_values, stop_data_point):
    ...
    s = int(stop_data_point / 2048)
    
    reshaped_accel_type_values = np.reshape(accel_type_array[:s*2048], (s, 2048))
    reshaped_accel_type_values_cleaned = np.delete(reshaped_accel_type_values, mean_velocity_indices[0][:-2], axis=0)
    print(reshaped_accel_type_values_cleaned.shape)
...
# plot spectrogram
i=0
flattened_size_defect = flattened_accel_x_defect.shape[0]
while i < flattened_size_defect:
    pxx, freq, t, cax_x = ax.specgram(flattened_accel_x_defect[i:i + dp], # first channel
                                    Fs=200,                            # to get frequency axis in Hz
                                                                        cmap=cmap, vmin=-120, vmax=0)
    plt.savefig("../stacked_defect2/x/" + str(int(i/dp)) + ".png")
    img_x = mpimg.imread("../stacked_defect2/x/" + str(int(i/dp)) + ".png")
    img_x_arr = img_x[:,:,0]
    
    pxx, freq, t, cax_y = ax.specgram(flattened_accel_y_defect[i:i + dp], # first channel
                                    Fs=200,                            # to get frequency axis in Hz
                                    cmap=cmap, vmin=-120, vmax=0)
    plt.savefig("../stacked_defect2/y/" + str(int(i/dp)) + ".png")
    img_y = mpimg.imread("../stacked_defect2/y/" + str(int(i/dp)) + ".png")
    img_y_arr = img_y[:,:,0]
    
    pxx, freq, t, cax_z = ax.specgram(flattened_accel_z_defect[i:i + dp], # first channel
                                    Fs=200,                            # to get frequency axis in Hz
                                    cmap=cmap, vmin=-120, vmax=0)
    plt.savefig("../stacked_defect2/z/" + str(int(i/dp)) + ".png")
    img_z = mpimg.imread("../stacked_defect2/z/" + str(int(i/dp)) + ".png")
    img_z_arr = img_z[:,:,0]
    
    stacked = np.dstack((img_x_arr, img_y_arr, img_z_arr))
    matplotlib.image.imsave("../dataset/defect/" + str(int(i/dp)) + ".png", stacked)
    i = i + dp
...
# plot spectrogram
i=0
flattened_size_intact = flattened_accel_x_intact.shape[0]
while i < flattened_size_intact:
    pxx, freq, t, cax_x = ax.specgram(flattened_accel_x_intact[i:i + dp], # first channel
                                    Fs=200,                            # to get frequency axis in Hz
                                    cmap=cmap, vmin=-120, vmax=0)
    plt.savefig("../stacked_intact/x/" + str(int(i/dp)) + ".png")
    img_x = mpimg.imread("../stacked_intact/x/" + str(int(i/dp)) + ".png")
    img_x_arr = img_x[:,:,0]
    
    pxx, freq, t, cax_y = ax.specgram(flattened_accel_y_intact[i:i + dp], # first channel
                                    Fs=200,                            # to get frequency axis in Hz
                                    cmap=cmap, vmin=-120, vmax=0)
    plt.savefig("../stacked_intact/y/" + str(int(i/dp)) + ".png")
    img_y = mpimg.imread("../stacked_intact/y/" + str(int(i/dp)) + ".png")
    img_y_arr = img_y[:,:,0]
    
    pxx, freq, t, cax_z = ax.specgram(flattened_accel_z_intact[i:i + dp], # first channel
                                    Fs=200,                            # to get frequency axis in Hz
                                    cmap=cmap, vmin=-120, vmax=0)
    plt.savefig("../stacked_intact/z/" + str(int(i/dp)) + ".png")
    img_z = mpimg.imread("../stacked_intact/z/" + str(int(i/dp)) + ".png")
    img_z_arr = img_z[:,:,0]
    
    stacked = np.dstack((img_x_arr, img_y_arr, img_z_arr))
    matplotlib.image.imsave("../dataset/intact/" + str(int(i/dp)) + ".png", stacked)
    i = i + dp
